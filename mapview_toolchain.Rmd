...
Supporting authors: Edzer Pebesma, Christoph Reudenbach, (Kenton Russel)
...
An integral part of any spatial data analysis workflow is visual inspection of intermediate analysis results. Virtually every open or closed source geospatial software provides an interactive data visualisation module where visualisation is routinely updated, usually after each step in the analysis. These visual modules usually provide easy access to the data by providing layered plots as spatial data usually has multiple attributes attached to the same geometry which can be changed with one or two mouse clicks. This makes visual exploration of complex spatial data easy and quick. Such quick visual inspection capabilities are currently lacking in R. Package [mapview](https://cran.r-project.org/web/packages/mapview/index.html) was created to fill this gap. It is in large parts built on top of the `leaflet` package and enables interactive visualisation of multiple layers with one or only a few lines of code, e.g. `mapview(x, burst = TRUE) + y` will visualise all attributes of spatial object `x` as individual layers and additionally add a layer of the locations of object `y`. Furthermore, all features of `x` and `y` are fully queryable via popups. As such, it basically provides an interactive version of `plot()` or `spplot()` for spatial data. As spatial data can quickly grow to large proportions of many tens of thousands of features, `mapview` also includes some special functionality to cope with such amounts, as `leaflet` based maps become rather unresponsive when data sets are large.
...
In general, we are aiming for complete `leaflet` compatibility so that the developped functionality will benefit the general community by providing interactive reactivity for a wide range of end users, i.e. not limted to users working with `mapview`. 
...
4. ensure that the proposed two-way data flow is compatible with all relevant spatial classes in R, including `sp`, `raster` and the currently developped `simpleFeatures` (M3-6)
...
1. Failure mode: **as far as I see it there is no anticipated failure is there?**
